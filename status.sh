...
Download_Server_Status_server(){
	cd "/tmp"
	wget -N --no-check-certificate "${Ghproxy}https://github.com/stilleshan/ServerStatus/archive/master.zip"
...
Download_Server_Status_client(){
	cd "/tmp"
	wget -N --no-check-certificate "${Ghproxy}https://raw.githubusercontent.com/stilleshan/ServerStatus/master/clients/status-client.py"
...
Service_Server_Status_server(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "${Ghproxy}https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/service/server_status_server_centos" -O /etc/init.d/status-server; then
		...
		if ! wget --no-check-certificate "${Ghproxy}https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/service/server_status_server_debian" -O /etc/init.d/status-server; then
			echo -e "${Error} ServerStatus 服务端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status-server
...
Service_Server_Status_client(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "${Ghproxy}https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/service/server_status_client_centos" -O /etc/init.d/status-client; then
		...
		if ! wget --no-check-certificate "${Ghproxy}https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/service/server_status_client_debian" -O /etc/init.d/status-client; then
			echo -e "${Error} ServerStatus 客户端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status-client
...
Install_jq(){
	if [[ ! -e ${jq_file} ]]; then
		if [[ ${bit} = "x86_64" ]]; then
			wget --no-check-certificate "${Ghproxy}https://github.com/stedolan/jq/releases/download/jq-1.5/jq-linux64" -O ${jq_file}
		else
			wget --no-check-certificate "${Ghproxy}https://github.com/stedolan/jq/releases/download/jq-1.5/jq-linux32" -O ${jq_file}
		fi
...
Install_caddy(){
	...
	if [[ "${caddy_yn}" == [Yy] ]]; then
		...
		if [[ ! -e "/usr/local/caddy/caddy" ]]; then
			wget -N --no-check-certificate ${Ghproxy}https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/caddy_install.sh
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "${Ghproxy}https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/status.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	wget -N --no-check-certificate "${Ghproxy}https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/status.sh" && chmod +x status.sh
